...
STAI_NETWORK_CONTEXT_DECLARE(m_embed_network, STAI_EMBED_NETWORK_CONTEXT_SIZE)

/* Array to store the data of the activation buffers */
STAI_ALIGNED(STAI_EMBED_NETWORK_ACTIVATION_1_ALIGNMENT)
static uint8_t activations_1[STAI_EMBED_NETWORK_ACTIVATION_1_SIZE_BYTES];
...
static stai_ptr m_inputs[STAI_EMBED_NETWORK_IN_NUM];
static stai_ptr m_outputs[STAI_EMBED_NETWORK_OUT_NUM];
static stai_ptr m_acts[STAI_EMBED_NETWORK_ACTIVATIONS_NUM];
...
int aiInit(void)
{
  ...
  stai_embed_network_init(m_embed_network);
  ...
  stai_embed_network_set_activations(m_embed_network, m_acts, STAI_EMBED_NETWORK_ACTIVATIONS_NUM);
  stai_embed_network_get_activations(m_embed_network, m_acts, &_dummy);
  ...
  stai_embed_network_get_inputs(m_embed_network, m_inputs, &_dummy);

  /* Output buffers are allocated in the activations buffer */
  stai_embed_network_get_outputs(m_embed_network, m_outputs, &_dummy);
...
stai_return_code aiRun()
{
  ...
  res = stai_embed_network_run(m_embed_network, STAI_MODE_SYNC);
