...
int main(void)
{
  ...
  MX_I2C2_Init();
  /* USER CODE BEGIN 2 */
   // setvbuf(stdout, NULL, _IONBF, 0);    // this line was required to be added in lab07, but when added here, for some reason, it stops board-to-pc communication. Ignore for now.
  
  // ---- IMU Sensor (ACC + GYRO) ----
  BSP_MOTION_SENSOR_Init(0U, MOTION_ACCELERO | MOTION_GYRO);
  
  // Configure Accelerometer (26 Hz, 4g)
  BSP_MOTION_SENSOR_SetOutputDataRate(0U, MOTION_ACCELERO, 26.0f);
  BSP_MOTION_SENSOR_SetFullScale(0U, MOTION_ACCELERO, 4);

  // Configure Gyroscope (26 Hz, 2000 dps full scale)
  BSP_MOTION_SENSOR_SetOutputDataRate(0U, MOTION_GYRO, 26.0f);
  BSP_MOTION_SENSOR_SetFullScale(0U, MOTION_GYRO, 2000);
  ...
  // initialize button in EXTI interrupt mode
  BSP_PB_Init(BUTTON_USER, BUTTON_MODE_EXTI);

  // prime the UART receive interrupt
